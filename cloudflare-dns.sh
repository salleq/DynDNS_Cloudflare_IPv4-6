...
settings_domains_validation() {
  ...
  for ((i = 0; i < n_doms; i++)); do
    ...
    if [ "${domains__proxied[i]}" != "false" ] &&
      ...
      def_proxied_enabled[i]=true
    else
      def_proxied_enabled[i]=false
    fi
    ...
    if [ "${domains__ip_type[i]}" != "external" ] &&
      ...
      def_ip_type_enabled[i]=true
    else
      def_ip_type_enabled[i]=false
    fi
...
telegram_notification() {
  ...
  if [ "${notifications_telegram_enabled}" == true ]; then
    telegram_notification=$(api_telegram)
